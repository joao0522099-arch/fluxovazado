            ...
            {isAdmin && (
                <div className="toggle-row">
                    <div className="toggle-info">
                        <h4 className="font-semibold text-white">Aprovar Membros</h4>
                        <p className="text-xs text-gray-400">Solicitações requerem aprovação</p>
                    </div>
            ...
            {pendingRequests.length > 0 && (
                <div className="mb-4 mt-2">
                    ...
                    {pendingRequests.map(u => (
                        <div key={u.id} className="flex justify-between items-center bg-white/5 p-2 rounded mb-1">
                            <span className="text-sm font-medium">{u.profile?.name}</span>
                            <div className="flex gap-2">
                                <button onClick={() => handlePendingAction(u.id, 'accept')} className="text-[#00ff82]"><i className="fa-solid fa-check"></i></button>
                                <button onClick={() => handlePendingAction(u.id, 'deny')} className="text-[#ff4d4d]"><i className="fa-solid fa-xmark"></i></button>
                            </div>
                        </div>
                    ))}
                </div>
